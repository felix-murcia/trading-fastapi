...
async def prepare(req: OrderPrepareRequest) -> OrderPrepareResponse:
    ...
    from services.risk_guardian import check_risk
    from services.mt5_client import get_account, get_positions
    try:
    ...
    dup = await pool.fetchrow(
        "SELECT id FROM orders WHERE symbol=$1 AND status IN ('pending','placed') "
        "AND created_at >= NOW() - INTERVAL '" + str(settings.order_max_age_hours) + " hours' "
        "AND ABS(entry - $2) <= $3",
