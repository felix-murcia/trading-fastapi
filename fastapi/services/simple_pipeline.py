    ...
    try:
        ...
        if time.time() - last < cooldown_secs:
            await _audit(cycle_id, "simple_skip", {
                "reason": "cooldown_active", "symbol": symbol, "direction": direction,
                "seconds_remaining": round(cooldown_secs - (time.time() - last)),
            })
            return {"action": "skip", "reason": "cooldown_active"}

        # 4. Filtro de noticias fundamentales (condicional por payload)
        if use_news_filter:
        ...
        # 4.5. Filtro gubernamental JPYUSD: limitar entradas long cerca de 163 (riesgo alto de manipulación)
        if symbol == "USDJPY" and direction == "buy" and price > 0 and price < 163.5:
            await _audit(cycle_id, "simple_skip", {
                "reason": "gov_risk_jpy_long_163", "symbol": symbol, "direction": direction,
                "price": price, "threshold": 163.5,
            })
            logger.info("[SIMPLE] SKIP %s — gov risk JPY long near 163: %.5f", symbol, price)
            return {"action": "skip", "reason": "gov_risk_jpy_long_163"}

        # 5. Order computation (Bollinger vs structural fallback)
        spread = await mt5_client.get_spread(symbol)
        if sl_anchor and tp_anchor:
            entry, sl, tp, volume = position_sizing.derive_order_from_bb(
                direction, symbol, price, sl_anchor, tp_anchor, spread
            )
        else:
            sl_anchor_tmp = signal_price if signal_price and signal_price > 0 else None
            if not sl_anchor_tmp:
                sl_anchor_tmp = await mt5_client.get_candle_open(symbol, timeframe="H1")
            
            entry, sl, tp, volume = position_sizing.derive_order_from_candle_open(
                direction, symbol, price, sl_anchor_tmp, spread
            )
        ...
        # 8. Marcar intento y enviar orden a MT5
